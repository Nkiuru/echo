...
const createUpload = (req, res, next) => {
  return new Promise((resolve, reject) => {
    db.createUpload(req.user.userId, req.file.filename, req.file.size, req.file.thumbnail).then((result) => {
...
const createSong = (req, res, next) => {
  return new Promise((resolve, reject) => {
    ...
    createUpload(req, res, next).then((uploadId) => {
      return db.createSong(req.body.title, undefined, req.body.genreId, uploadId, req.user.bandId);
    }).then((songId) => {
      resolve(songId);
    }).catch((err) => reject(err));
  });
...
const createImage = (req, file) => {
  return new Promise((resolve, reject) => {
    db.createUpload(req.user.userId, file.filename, file.size, file.thumbnail).then((result) => {
      return db.createImage(file.filename, result['LAST_INSERT_ID()'], '', req.imageAlbumId);
    }).then(() => resolve()
    ).catch((err) => reject(err));
  });
};
